...
## 1) 에코 서버: `hello`가 클라에서 서버로 갔다가 돌아오는 경로

현재 과제의 `echo_client.c`, `echo_server.c`, `echo_common.c`는 `hello` 텍스트를 입력받아 같은 내용으로 되돌려 주는 에코 동작을 수행한다.
아래처럼 1-1, 1-2에서 각각 함수 호출 흐름과 데이터 이동 경로를 분리해 본다.

```mermaid
flowchart TD
  subgraph C["클라이언트"]
    U["사용자 입력<br/>hello"]
    CMain["echo_client.c main"]
    CF1["Rio_writen(clientfd, buf, len)"]
    Ck1["클라 커널 send 버퍼"]
    Ck2["클라 커널 recv 버퍼"]
    CR["Rio_readlineb(&rio, buf, MAXLINE)"]
    COut["Fputs(buf, stdout)"]
  end

  subgraph Net["네트워크 경로"]
    N["NIC TX/RX + 스위치/라우터 중간 장치"]
  end

  subgraph S["서버"]
    SMain["echo_server.c main"]
    SCommon["echo_common.c<br/>echo_handle_one_line"]
    SRead["Rio_readlineb(&rio, buf, MAXLINE)"]
    Sk1["서버 커널 recv 버퍼"]
    Sk2["서버 커널 send 버퍼"]
  end

  U --> CMain --> CF1 --> Ck1 --> N --> Sk1 --> SMain --> SRead --> SCommon --> Sk2 --> N --> Ck2 --> CR --> COut
  CR --> CF1
```
