...
graph TD
  ...
  subgraph Net["네트워크"]
    K1["클라 send buffer"] --> N1["NIC / 스위치 / 라우터"]
    N1 --> K2["서버 recv buffer"]
    K3["서버 send buffer"] --> N2["NIC / 스위치 / 라우터"]
    N2 --> K4["클라 recv buffer"]
  end
...
- `C0`: 사용자가 입력한 문자열
- `C1`: 클라이언트의 시작점
- `C2`: `Open_clientfd()`로 서버와 통신할 소켓을 준비하는 단계
- `C3`: `fgets()`가 입력을 읽어 `buf`를 채우는 단계
- `C4`: `buf` 내용을 서버로 보내는 단계
- `C5`: 서버 응답을 다시 읽는 단계
- `C6`: 화면에 출력하는 단계
- `K1`: 클라이언트 커널의 송신 버퍼
- `N1`: 네트워크 구간
...
- `S1`: 서버 프로그램의 시작점
- `S2`: 서버용 소켓 준비 시작
- `S3`: 서버 주소를 소켓에 붙이는 `bind()` 단계
- `S4`: 연결 요청을 받기 시작하는 `listen()` 단계
- `S5`: 연결을 새로 받는 `accept()` 단계
- `S6`: 연결 하나를 처리하는 `echo()` 단계
- `S7`: 서버가 한 줄을 읽는 단계
- `S8`: 서버가 같은 내용을 다시 보내는 단계
- `K3`: 서버 커널의 송신 버퍼
- `N2`: 응답이 지나가는 네트워크 구간
- `K4`: 클라이언트 커널의 수신 버퍼
