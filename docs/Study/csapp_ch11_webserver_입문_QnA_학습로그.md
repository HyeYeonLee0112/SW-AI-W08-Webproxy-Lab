...
## 2026-04-21 Q9. `fork` 방식 vs `thread pool` 방식 비교

### 질문
- Tiny처럼 `fork()`로 자식 프로세스를 여러 개 만들어 병렬 처리하는 방식과, `thread pool`로 병렬 처리하는 방식은 어떤 차이가 있는가?
- 왜 어떤 경우에는 `fork()`보다 `thread pool`을 더 많이 쓰는가?
- 둘 중 하나가 항상 더 좋은가?

### 비교표

| 항목 | `fork` 방식 | `thread pool` 방식 |
|---|---|---|
| 작업 단위 | 요청마다 자식 프로세스를 만들기 쉬움 | 미리 만든 작업자 스레드를 재사용함 |
| 생성 비용 | 비교적 큼 | 비교적 작음 |
| 메모리 공간 | 프로세스마다 분리됨 | 같은 프로세스 메모리를 공유함 |
| 장애 격리 | 강함. 한 자식이 죽어도 부모/다른 자식에 영향이 비교적 적음 | 약함. 한 스레드의 문제는 같은 프로세스 전체에 영향 가능 |
| 데이터 공유 | 어렵고 별도 통신이 필요할 수 있음 | 쉽다. 공유 메모리를 바로 쓸 수 있음 |
| 동기화 | 상대적으로 단순 | 락, 경합, 데드락, 레이스 조건 관리가 필요함 |
| 동시 연결 규모 | 많아질수록 부담이 커질 수 있음 | 보통 더 유리함 |
| 디버깅 | 구조가 비교적 단순함 | 동시성 버그 추적이 어려움 |
| 대표적인 인상 | 격리 중심, 직관적 구조 | 효율 중심, 재사용 중심 |

### 핵심 개념
- `fork`는 **격리와 단순성**이 강점이다.
- `thread pool`은 **효율과 확장성**이 강점이다.
- `fork()`가 무거운 것은 맞지만, 그 한 가지 이유만으로 `thread pool`이 항상 더 낫다고 말할 수는 없다.
- 실제 선택은 `성능`, `격리`, `공유 상태`, `동시성 규모`, `구현 난이도`를 같이 본다.

### 언제 어떤 쪽이 더 유리한가
- `fork`가 잘 맞는 경우
  - 구조를 단순하게 보여주고 싶을 때
  - 프로세스 격리가 중요할 때
  - 학습용 예제처럼 흐름을 명확히 드러내고 싶을 때
- `thread pool`이 잘 맞는 경우
  - 요청 수가 많고, 작업자 재사용이 중요할 때
  - 프로세스 생성 비용을 줄이고 싶을 때
  - 공유 데이터 접근이 잦을 때

### 오해 포인트 교정
- "프로세스가 무거우니까 무조건 스레드 풀이 정답"은 아니다.
- 스레드 풀은 프로세스보다 가볍지만, 그 대신 공유 메모리 때문에 동기화 문제가 생긴다.
- `fork`는 무겁지만 격리가 강하다.
- 둘은 경쟁 관계라기보다, **문제 성격에 따라 선택이 달라지는 두 방식**이다.

### 한 줄 정리
- `fork`는 "서로 분리된 자식 프로세스들이 각자 처리하는 방식"이고, `thread pool`은 "미리 만들어 둔 작업자 스레드들이 요청을 나눠 처리하는 방식"이다.
- Tiny 같은 학습용 서버에서는 `fork`가 개념을 보여주기 좋고, 실무에서는 `thread pool`이나 이벤트 기반 방식이 더 자주 선택되는 경우가 많다.
